...
// static std::string clsNmae[10] = {"pedestrian", "people", "bicycle", "car", "van", "truck", "tricycle", "awn-tricycle", "bus", "motor"};
static std::string clsNmae[] = {"person", "vehicle","pedestrian", "people", "bicycle", "car", "van"};
// static std::string clsNmae[5] = {"car", "truck","bus","van","freight car"};
...
void draw_bbox(std::vector<cv::Mat>& img_batch, std::vector<std::vector<Detection>>& res_batch) {
  for (size_t i = 0; i < img_batch.size(); i++) {
    ...
    for (size_t j = 0; j < res.size(); j++) {
      ...
      // printf(")res[j].class_id:%d\n", res[j].class_id);
      // cv::putText(img, clsNmae[(int)res[j].class_id], cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 1);
    }
  }
}
